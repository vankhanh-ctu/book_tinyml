...
TfLiteStatus InitializeMicroFeatures(tflite::ErrorReporter* error_reporter) {
  ...
  config.filterbank.lower_band_limit = 50.0;
  config.filterbank.upper_band_limit = 7925.0;
  config.noise_reduction.smoothing_bits = 10;
...
TfLiteStatus GenerateMicroFeatures(tflite::ErrorReporter* error_reporter,
  ...
  if (g_is_first_time) {
    ...
    frontend_input = input + kFeatureSliceStrideMs*(kAudioSampleFrequency/1000);
  }
  FrontendOutput frontend_output = FrontendProcessSamples(
      &g_micro_features_state, frontend_input, input_size, num_samples_read);
for(size_t i = 0; i < frontend_output.size; ++i){
 if(max_2<frontend_output.values[i]){
  max_2 = frontend_output.values[i];
 }
}
  for (int i = 0; i < frontend_output.size; ++i) {
   ...
   int32_t value =(frontend_output.values[i]/((float)max_2/255.0f))-128;

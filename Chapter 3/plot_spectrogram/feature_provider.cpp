...
TfLiteStatus FeatureProvider::PopulateFeatureData(
    ...
    int32_t time_in_ms, int* how_many_new_slices, int16_t data_raw[]) {
  ...
  *how_many_new_slices = slices_needed;
  ...
  const int slices_to_drop = kFeatureSliceCount - slices_to_keep;
   if (slices_to_keep > 0) {
    ...
    audio_samples_size = kFeatureSliceDurationMs*kAudioSampleFrequency/1000;
    for (int new_slice = slices_to_keep; new_slice < kFeatureSliceCount;
      ...
      const int32_t slice_start_sample =  slice_start_ms*kAudioSampleFrequency/1000;
      int16_t audio_samples[audio_samples_size];
      int sum=0;
      float mean=0;
      for(int i=0;i<audio_samples_size;i++){
        audio_samples[i] =data_raw[i + slice_start_sample ];

...
TfLiteStatus FeatureProvider::PopulateFeatureData(
  ...
  const float remain = (((float)time_in_ms /(float)kFeatureSliceStrideMs)-((float)last_time_in_ms / (float)kFeatureSliceStrideMs))-(current_step-last_step);
...
//         {
//          if (max1<abs(audio_samples[i])) max1=abs(audio_samples[i]);  
//          }
//        for(int i=0;i<kMaxAudioSampleSize;i++)
//         {
//          audio_samples[i]=((float)audio_samples[i]/max1)*32768;  
//          }
//          for(int i=0;i<kMaxAudioSampleSize;i++)
//         {

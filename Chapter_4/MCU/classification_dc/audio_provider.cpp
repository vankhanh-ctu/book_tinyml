...
static void CaptureSamples(void* arg) {
  ...
  while (1) {
    ...
    i2s_read(I2S_NUM_0, (void*)i2s_read_buffer, i2s_bytes_to_read, &bytes_read, 10);//sizeof(int32_t) * SAMPLE_BUFFER_SIZE
...
//          if(g_latest_audio_timestamp>=69000 ) {rb_reset(g_audio_capture_buffer);}//g_latest_audio_timestamp
//      Serial.print("fill_cnt:");
//      Serial.println(g_audio_capture_buffer->fill_cnt);
//      Serial.print("size:");
//      Serial.println(g_audio_capture_buffer->size);
//      Serial.print("time:");
...
void Clear_remain(int duration_ms) {
...
int32_t LatestTimes() { return g_latest_audio_timestamp; }
